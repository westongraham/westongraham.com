...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return <main className="page-shell editorial-page case-study"><JsonLd data={{ "@context": "https://schema.org", "@type": "CreativeWork", name: study.title, description: study.summary, url: absoluteUrl(`/projects/${study.slug}`), author: { "@type": "Person", name: "Weston Graham", url: absoluteUrl("/") }, keywords: study.technologyStack }} /><Link className="back-link" href="/projects">← All projects</Link><p className="section-label">{study.eyebrow}</p><h1>{study.title}</h1><p className="lede">{study.summary}</p>{study.demo?.url && <div className="case-actions"><a className="primary-action" href={study.demo.url} target="_blank" rel="noreferrer">{study.demo.label ?? "View live site"} <span aria-hidden="true">↗</span></a></div>}<ProjectTemplate study={study} /><nav className="next-project" aria-label="Next project"><h2 className="section-label">Next project</h2><Link href={`/projects/${nextStudy.slug}`}><span>{nextStudy.eyebrow}</span><strong>{nextStudy.title} <i aria-hidden="true">→</i></strong></Link></nav></main>;
}
